...
const std::string keys =
  ...
  "{@config-path   | configs/gkdhero.yaml | 位置参数，yaml配置文件路径 }";
...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    if (!targets.empty()) {
      ...
      tools::logger()->info(
        "[Hero][{}] Target state -> x {:.3f} m, y {:.3f} m, z {:.3f} m, yaw {:.3f} rad",
        frame_count, state[0], state[2], state[4], state[6]);
    }

    tools::logger()->info(
      "[Hero][{}] yolo: {:.1f}ms, tracker: {:.1f}ms, aimer: {:.1f}ms", frame_count,
      tools::delta_time(tracker_start, yolo_start) * 1e3,
      tools::delta_time(aimer_start, tracker_start) * 1e3,
      tools::delta_time(finish, aimer_start) * 1e3);

    cv::resize(img, img, {}, 0.5, 0.5);
    cv::imshow("gkdhero", img);
    cv::waitKey(1);
